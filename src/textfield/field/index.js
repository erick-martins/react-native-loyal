...
export default class TextField extends BaseTextComponent {
  focusStyle = {};
  disabledStyle = {
    inputContainerStyle: {
  ...
  _checkText(text) {
    const { checkText } = this.props;

    if ('function' === typeof checkText) {
      return checkText(this.state.text, text);
    }
    return true;
  }

  isMaskSet() {
    return this._maskHandler != null;
  }

  _getKeyboardType() {
    return (
      this.props.keyboardType ||
      (this._maskHandler && this._maskHandler.getKeyboardType()) ||
      'default'
    );
  }
  //<-- FROM MASK
  componentWillReceiveProps(props) {
    ...
    this.setState({ errored: props.error ? !!props.error : false });

    if (props.success !== success) {
      this.setState({ success: props.success });
    }
    this.setState({ successful: props.success ? !!props.success : false });
  }
  ...
  componentWillUpdate(props, state) {
    let { error, success, animationDuration: duration } = this.props;
    ...
    if (
      ...
      let toValue = this.focusState(props.error, props.success, state.focused);
    ...
    // if (props.success !== success || focused ^ state.focused) {
    //   let toValue = this.focusState(props.success, state.focused);
  ...
  focusState(error, success, focused) {
    return error ? -1 : success ? 2 : focused ? 1 : 0;
  }
    ...
    if (this.isMaskSet()) {
      ...
      self.updateValue(text).then(maskedText => {
        if ('function' === typeof onChangeText) {
          this.getElement().setNativeProps({ value: maskedText });
          this.setState({ text: maskedText });
  ...
  onFocusAnimationEnd() {
    if (this.mounted) {
      this.setState((state, { error, success }) => ({ error, success }));
    }
  }
  ...
  render() {
    ...
    // MASK SHIT
    let _maxChar = limit;
    let _showCounter = showCounter;
    let _limitToMaxChar = limitToMaxChar;

    if (this.isMaskSet()) {
      if (this._maskHandler.maxChar() > -1) {
        _maxChar = this._maskHandler.maxChar();
        _showCounter = false;
        _limitToMaxChar = true;
      }
    }

    //debugger;

    if (_maxChar && _maxChar < count && limitToMaxChar) {
      value = value.substring(0, _maxChar);
      props.maxLength = _maxChar;
    }
    let restricted = _maxChar < count;
    ...
    let borderBottomColor = restricted
      ...
      : focus.interpolate({
          inputRange: [-1, 0, 1, 2],
          outputRange: [errorColor, lineColor, tintColor, successColor],
        });
        ...
        <Animated.View {...inputContainerProps}>
          ...
          <View style={styles.row}>
            ...
            <TextInput
              ...
              onBlur={this.onBlur}
              value={value}
              {...(_limitToMaxChar ? { maxLength: _maxChar } : {})}
              ref={this.updateRef}
            />
