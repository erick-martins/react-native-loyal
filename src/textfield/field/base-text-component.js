...
export default class BaseTextComponent extends ThemeComponent {
  constructor(props) {
    super(props);
    this.state = {
      type: props.type,
  ...
  getRawValue() {
    if (!this._maskHandler) return this.state.value;
  ...
  _mustUpdateValue(newValue) {
    if (!this._maskHandler) return false;
  ...
  _getDefaultMaskedValue(value) {
    if (!this._maskHandler) return value;
